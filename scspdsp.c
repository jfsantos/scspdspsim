...
/**
 * @brief Disassembles a single DSP instruction and prints a human-readable
 * representation.
 *
 ...
 *
 * @param IPtr A pointer to the first of the four 16-bit words of the
 * instruction.
...
void SCSPDSP_Disassemble(uint16_t *IPtr, int step_index) {
  ...
  instr.nofl = (IPtr[3] >> 15) &
               1; // No Floating-Point conversion (direct 16-bit access)
  ...
  printf("MPRO[%02X]: %04X %04X %04X %04X  | ", step_index, IPtr[0], IPtr[1],
         IPtr[2], IPtr[3]);
  ...
  if (instr.iwt) {
    sprintf(temp_str, "MEMS[0x%02X] = MEMVAL; ",
            instr.iwa); // MEMVAL is from previous MRD
    strcat(disasm_str, temp_str);
  }
  sprintf(temp_str, "INPUTS = %s[0x%02X]; ",
          (instr.ira <= 0x1F) ? "MEMS" : ((instr.ira <= 0x2F) ? "MIXS" : "0"),
          instr.ira);
  strcat(disasm_str, temp_str);
  ...
  switch (instr.ysel) {
  case 0:
    sprintf(y_src, "FRC_REG");
    break;
  case 1:
    sprintf(y_src, "COEF[0x%02X]", instr.coef);
    break;
  case 2:
    sprintf(y_src, "Y_REG[13:0]");
    break; // (Y_REG >> 11) & 0x1FFF
  case 3:
    sprintf(y_src, "Y_REG[11:0]");
    break; // (Y_REG >> 4) & 0x0FFF
  default:
    sprintf(y_src, "UNKNOWN");
    break;
  }
  ...
  if (instr.zero) {
    ...
    sprintf(b_src, "%s%s", instr.negb ? "-" : "",
            instr.bsel ? "ACC" : "TEMP[TRA+DEC]");
  }
  ...
  switch (instr.shift) {
  case 0:
    shift_op = "SATURATE(ACC)";
    break;
  case 1:
    shift_op = "SATURATE(ACC * 2)";
    break;
  case 2:
    shift_op = "(ACC * 2) & 0xFFFFFF";
    break; // Shift left 1, then sign extend to 24-bit
  case 3:
    shift_op = "ACC & 0xFFFFFF";
    break; // No shift, just sign extend to 24-bit
  default:
    shift_op = "UNKNOWN_SHIFT(ACC)";
    break;
  }
  ...
  if (instr.mrd || instr.mwt) {
    ...
    sprintf(mem_addr_str, "MADRS[0x%02X]%s%s%s", instr.masa,
            instr.table ? "" : "+DEC", instr.adreb ? "+ADREG" : "",
            instr.nxadr ? "+1" : "");

    if (instr.mrd) {
      sprintf(temp_str, "MR MEMVAL = SCSPRAM[%s]%s; ", mem_addr_str,
              instr.nofl ? " (Direct)" : " (Unpack)");
      strcat(disasm_str, temp_str);
    }
    if (instr.mwt) {
      sprintf(temp_str, "MW SCSPRAM[%s] = SHIFTED%s; ", mem_addr_str,
              instr.nofl ? " (Direct)" : " (Pack)");
      strcat(disasm_str, temp_str);
    }
  }
  if (instr.ewt) {
...
void SCSPDSP_DisassembleProgram(struct _SCSPDSP *DSP) {
  ...
  for (int step = 0; step < 128; ++step) {
    // Only disassemble if the instruction is not all zeros (or up to LastStep
    // if available) For a full disassembly, we can iterate all 128 steps. If
    // you want to limit to actual program length, use DSP->LastStep. For this
    // disassembler, showing all potential steps is more informative.
...
/**
 * @brief Loads a DSP program from a .EXC file into the DSP program memory
 * (MPRO).
 *
 ...
 * where [data] consists of four 4-hex-digit blocks separated by spaces (e.g.,
...
int SCSPDSP_LoadProgramFromEXL(struct _SCSPDSP *DSP, const char *filename) {
  ...
  while (fgets(line, sizeof(line), file) != NULL) {
    ...
    if (strcmp(line, "COEF") == 0 || strcmp(line, "ADRS") == 0 ||
        strcmp(line, "END") == 0) {
      // If we encounter another section marker or END, and we were in PROG,
      // stop parsing PROG. If we encounter it before PROG, it's just skipping.
      if (in_prog_section) {
        break;
      }
      continue; // Move to the next line
    ...
    if (in_prog_section) {
      ...
      parsed_items = sscanf(line, "%x:%x %x %x %x", &address, &word0, &word1,
                            &word2, &word3);
      ...
      if (parsed_items == 5) {
        ...
        if (address >= 0 && address < 128) {
          ...
          fprintf(stderr,
                  "Warning: Line %d: Program address 0x%X out of bounds "
                  "(0-127). Skipping.\n",
                  line_num, address);
        }
      } else {
        // If parsing failed, it might be a comment or an empty line, or a
        // malformed instruction line. Ignore comments (lines starting with ';')
        // or empty lines. The dAsms manual says comments start with apostrophe
        // (').
        if (line[0] == '\'' || line[0] == '\0' || line[0] == '\r') {
          continue; // Skip comments and empty lines
        }
        fprintf(stderr,
                "Error: Line %d: Malformed PROG instruction line: '%s'. "
                "Expected 'ADDR:WORD0 WORD1 WORD2 WORD3'.\n",
                line_num, line);
        fclose(file);
        return -1;
      }
 ...
 *
 * This function simulates the DSP's operation over a given number of audio
 * samples. For each sample, it feeds the input data to the DSP's mixers,
 * executes one DSP step, and then retrieves the processed output from the
 * effect registers.
 *
 * @param DSP A pointer to the SCSPDSP structure with the loaded program.
 * @param input_l Pointer to the input PCM data buffer for the left channel
 * (16-bit signed).
 * @param input_r Pointer to the input PCM data buffer for the right channel
 * (16-bit signed).
 * @param output_l Pointer to the output PCM data buffer for the left channel
 * (16-bit signed).
 * @param output_r Pointer to the output PCM data buffer for the right channel
 * (16-bit signed).
 * @param num_samples The number of audio samples to process.
 */
void SCSPDSP_Run(struct _SCSPDSP *DSP, const INT16 *input_l,
                 const INT16 *input_r, INT16 *output_l, INT16 *output_r,
                 int num_samples) {
  ...
  // FIXME this whole implementation assumes we are feeding stereo data in and
  // out. Linker actually allows us to run more inputs and outputs to DSP code.
  for (int i = 0; i < num_samples; ++i) {
    // Feed input samples to the DSP's MIXS registers.
    // Assuming MIXS[0] for left and MIXS[1] for right, based on common stereo
    // setups and the dAsms example using EXTS00 and EXTS01 for L/R input. The
    // SCSPDSP_SetSample function adds the sample to MIXS.
    ...
    // The dAsms example outputted to EFREG00 for L and EFREG01 for R (stereo
    // out). EFREG stores 24-bit values (shifted by 8 from SHIFTED), so convert
    // back to 16-bit. Clamp to INT16 range (-32768 to 32767)
...
/**
 * @brief Reads 16-bit signed PCM data from a binary file into a dynamically
 * allocated buffer.
 *
 * @param filename The path to the PCM file.
 * @param buffer A pointer to an INT16* that will store the allocated buffer.
 * The caller is responsible for freeing this memory.
 * @param num_samples A pointer to a long that will store the number of samples
 * read.
...
int read_pcm_file(const char *filename, INT16 **buffer, long *num_samples) {
  ...
  if (file_size % sizeof(INT16) != 0) {
    fprintf(stderr,
            "Warning: PCM file '%s' size (%ld bytes) is not a multiple of "
            "sample size (%zu bytes). Truncating.\n",
  ...
  *buffer = (INT16 *)malloc(*num_samples * sizeof(INT16));
  if (!*buffer) {
    fprintf(stderr,
            "Error: Failed to allocate memory for PCM buffer for '%s'.\n",
            filename);
    fclose(file);
    *num_samples = 0;
    return -1;
 ...
 * @return 0 on success, -1 on failure.
 */
int write_pcm_file(const char *filename, const INT16 *buffer,
                   long num_samples) {
...
void print_usage(const char *prog_name) {
  ...
  fprintf(stderr,
          "  %s run <program.exc> <input_l.pcm> <input_r.pcm> <output_l.pcm> "
          "<output_r.pcm>\n",
          prog_name);
  fprintf(stderr, "\n");
  fprintf(stderr,
          "  disasm: Disassembles the DSP program from the .exc file.\n");
  fprintf(stderr, "  run:    Runs the DSP program with input PCM files and "
                  "writes to output PCM files.\n");
  fprintf(
      stderr,
      "          PCM files are assumed to be raw 16-bit signed stereo data.\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "disasm") == 0) {
    if (argc != 3) {
      fprintf(stderr,
              "Error: Incorrect number of arguments for 'disasm' mode.\n");
      print_usage(argv[0]);
      return EXIT_FAILURE;
    }
  ...
  // --- Run Mode ---
    ...
    const char *output_r_file =
        argv[6]; // This should be argv[5] and argv[6] if argc is 6.
                 // Correction: argv[5] is the 5th argument, so output_r_file
                 // should be argv[6] Let's re-check argc. If run takes 5
                 // arguments after "run", then argc should be 1 + 1 + 5 = 7.
                 // The current check `argc != 6` means `program.exc`,
                 // `input_l`, `input_r`, `output_l`, `output_r` are 4
                 // arguments. It should be `argc != 7`.
    ...
    if (num_samples_l != num_samples_r) {
      fprintf(
          stderr,
          "Error: Input PCM files have different sample counts (%ld vs %ld).\n",
    ...
    if (!output_l_buffer || !output_r_buffer) {
      fprintf(stderr,
              "Error: Failed to allocate memory for output PCM buffers.\n");
      free(input_l_buffer);
      free(input_r_buffer);
    ...
    SCSPDSP_Run(myDsp, input_l_buffer, input_r_buffer, output_l_buffer,
                output_r_buffer, num_samples_l);
